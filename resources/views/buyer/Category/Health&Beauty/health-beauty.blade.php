@php
    $catalog = json_decode(
        file_get_contents(resource_path('data/buyer-health-beauty-products.json')),
        true,
        512,
        JSON_THROW_ON_ERROR
    );
    ...
    $healthBeautyCategory = collect($taxonomy)->firstWhere('slug', 'health-and-beauty');

    $subcategories = $healthBeautyCategory['subcategories'] ?? [
        'Skin Care',
        'Hair Care',
        'Makeup',
        'Personal Care',
...
<head>
    ...
    <title>Health & Beauty | Bearly</title>
...
<body class="bc" style="--catalog-image:url('{{ asset('images/health-beauty-catalog-atlas.png') }}')">
    ...
    <header class="bc-header">
        ...
        <form id="bc-search-form" class="search" role="search">
            ...
            <label class="sr" for="bc-search">Search Health & Beauty</label>
            <input id="bc-search" type="search" placeholder="Search Health & Beauty" maxlength="120">
    ...
    <div class="bc-shell">
        <aside class="bc-sidebar" id="bc-sidebar" aria-label="Categories and filters">
            ...
            <div class="active-category">
                <i class="mi" aria-hidden="true">spa</i>
                    ...
                    @foreach($subcategories as $i => $sub)
                        <button data-sub="{{ $sub }}" aria-pressed="false">
                            <i class="mi" aria-hidden="true">
                                {{ ['spa','content_cut','brush','sanitizer','dentistry','health_and_safety'][$i] ?? 'spa' }}
                            </i>
                            {{ $sub }}
                        </button>
                    @endforeach
            ...
            @include('buyer.Category.Health&Beauty.components.health-beauty-filter')
        </aside>
        ...
        <main id="bc-main" tabindex="-1">
            ...
            <p class="subtitle">
                Everyday care, beauty, and wellness essentials in one place.
            </p>
            ...
            <div class="shortcuts" aria-label="Explore subcategories">
                @php
                    $shortcutPhotos = [0, 10, 16, 24, 8, 25];
                @endphp
    ...
    @include('buyer.Category.Health&Beauty.components.health-beauty-product-card')
