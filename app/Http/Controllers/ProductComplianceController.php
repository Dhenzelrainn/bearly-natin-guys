...
{
    public function decide(Request $request, ProductViolation $violation): RedirectResponse
    {
        ...
        DB::transaction(function () use ($violation, $data, $request) {
            ...
            $status = $data['decision'] === 'dismiss' ? 'dismissed' : ($data['decision'] === 'request_correction' ? 'under_review' : 'confirmed');
