...
<div class="page-heading returns-page-heading">
    <div><span class="section-kicker">Order resolution</span><h2>Returns & Refunds</h2><p>Review buyer requests, submit seller evidence, and monitor the platform/logistics resolution.</p></div>
...
<section class="return-role-flow" aria-label="Return case ownership">
    ...
    <i data-lucide="arrow-right"></i>
    <div><span>4</span><strong>Logistics</strong><small>Handles physical return when needed</small></div>
</section>
            ...
            <tbody>
                @foreach ($returns as $case)
                    ...
                    <tr data-return-row data-status="{{ $case['status_key'] }}" data-type="{{ $typeKey }}" data-search="{{ strtolower(implode(' ', $case)) }}">
                        ...
                        <td><span class="return-finance-effect">{{ $case['commission_effect'] }}</span><small>{{ $case['earnings_effect'] }}</small></td>
