...
<title>Register as Rider | Bearly</title>
...
<div class="shell">
<section class="hero"><div class="brand">Bearly</div><div><div class="eyebrow">Rider Application</div><h1>Register under a Logistics partner.</h1><p>Select the approved sorting center or logistics company you will work under. That Logistics partner—not Bearly Admin—reviews and activates your Rider account.</p></div><p class="small">Approval authority: Selected Logistics / Sorting Center</p></section>
...
<p class="muted">Submit your rider and vehicle credentials.</p>
@if (session('registration_pending'))
<div class="notice success"><b>Rider application submitted.</b><br>Your selected Logistics / Sorting Center will review your credentials. Await approval before signing in.</div>
@endif
...
@csrf
<div class="form-grid">
<div class="field full"><label>Apply Under Logistics / Sorting Center</label>
<select name="logistics_partner" required>
<option value="">Select an approved Logistics partner</option>
@foreach ($logisticsPartners as $partner)
<option value="{{ $partner['id'] }}" @selected(old('logistics_partner') === $partner['id'])>{{ $partner['name'] }}</option>
@endforeach
</select></div>
<div class="field full"><label>Full Name</label><input name="full_name" value="{{ old('full_name') }}" required></div>
...
<div class="field"><label>Birthday</label><input type="date" name="birthday" id="riderBirthday" value="{{ old('birthday') }}" required></div>
<div class="field"><label>Age</label><input id="riderAge" readonly placeholder="Auto-calculated"></div>
...
<div class="field full"><label>Street / House Address</label><input name="street" value="{{ old('street') }}" required></div>
<div class="field"><label>Vehicle Type</label><select name="vehicle_type" required><option value="">Select</option><option>Motorcycle</option><option>Tricycle</option><option>Van</option><option>Other</option></select></div>
<div class="field"><label>Plate Number</label><input name="plate_number" value="{{ old('plate_number') }}" required></div>
<div class="field"><label>OR / CR</label><input type="file" name="or_cr" accept=".jpg,.jpeg,.png,.pdf" required></div>
<div class="field"><label>Driver License / Valid ID</label><input type="file" name="driver_license" accept=".jpg,.jpeg,.png,.pdf" required></div>
<div class="full"><div class="notice">Status after submission: <b>Awaiting Logistics / Sorting Center Approval</b>.</div></div>
<div class="full"><button class="btn btn-primary" style="width:100%" type="submit">Submit Rider Application</button></div>
</div></form>
</div></section></div>
<script>
const b=document.getElementById('riderBirthday'),a=document.getElementById('riderAge');
