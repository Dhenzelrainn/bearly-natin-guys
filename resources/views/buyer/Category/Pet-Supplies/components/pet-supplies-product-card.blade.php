<template id="bc-card-template">
    <article class="card">
        <div class="card-image">
            <button class="image-open" data-quick><span class="photo" role="img"></span></button>
            <button class="save" data-save aria-pressed="false"><i class="mi" aria-hidden="true">favorite</i></button>
        </div>
        <div class="card-copy">
            <h2></h2>
            <p class="condition"></p>
            <div class="price-row"><strong></strong><span class="color-label"></span></div>
            <button class="quick" data-quick><i class="mi" aria-hidden="true">visibility</i> Quick view</button>
        </div>
    </article>
</template>