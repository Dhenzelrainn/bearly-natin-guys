@php
    $catalog = json_decode(
        file_get_contents(resource_path('data/buyer-kids-baby-products.json')),
        true,
        512,
        JSON_THROW_ON_ERROR
    );
    ...
    $kidsCategory = collect($taxonomy)->firstWhere('slug', 'kids-and-baby');
...
<body class="bc" style="--catalog-image:url('{{ asset('images/kids-baby-catalog-atlas.png') }}')">
    ...
    <header class="bc-header">
        ...
        <form id="bc-search-form" class="search" role="search">
            ...
            <label class="sr" for="bc-search">Search Kids & Baby</label>
            <input id="bc-search" type="search" placeholder="Search Kids & Baby" maxlength="120">
    ...
    <div class="bc-shell">
        <aside class="bc-sidebar" id="bc-sidebar" aria-label="Categories and filters">
            ...
            <div class="active-category">
                <i class="mi" aria-hidden="true">child_care</i>
                    ...
                    @foreach($subcategories as $i => $sub)
                        <button data-sub="{{ $sub }}" aria-pressed="false">
                            <i class="mi" aria-hidden="true">
                                {{ ['child_care','checkroom','restaurant','crib','toys','directions_bike'][$i] ?? 'child_care' }}
                            </i>
                            {{ $sub }}
                        </button>
                    @endforeach
            ...
            @include('buyer.Category.Kids&Baby.components.kids-baby-filter')
        </aside>
        ...
        <main id="bc-main" tabindex="-1">
            ...
            <p class="subtitle">
                Everyday essentials for little ones, from playtime to bedtime.
            </p>
            ...
            <div class="shortcuts" aria-label="Explore subcategories">
                @php
                    $shortcutPhotos = [0, 2, 13, 17, 24, 28];
                @endphp
    ...
    @include('buyer.Category.Kids&Baby.components.kids-baby-product-card')
