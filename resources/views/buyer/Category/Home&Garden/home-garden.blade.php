@php
    $catalog = json_decode(
        file_get_contents(resource_path('data/buyer-home-garden-products.json')),
        true,
        512,
        JSON_THROW_ON_ERROR
    );
    ...
    $homeGardenCategory = collect($taxonomy)->firstWhere('slug', 'home-and-garden');

    $subcategories = $homeGardenCategory['subcategories'] ?? [
        'Garden & Plants',
        'Furniture',
        'Home Decor',
        'Kitchen & Storage',
...
<head>
    ...
    <title>Home & Garden | Bearly</title>
...
<body class="bc" style="--catalog-image:url('{{ asset('images/home-garden-catalog-atlas.png') }}')">
    ...
    <header class="bc-header">
        ...
        <form id="bc-search-form" class="search" role="search">
            ...
            <label class="sr" for="bc-search">Search Home & Garden</label>
            <input id="bc-search" type="search" placeholder="Search Home & Garden" maxlength="120">
    ...
    <div class="bc-shell">
        <aside class="bc-sidebar" id="bc-sidebar" aria-label="Categories and filters">
            ...
            <div class="active-category">
                <i class="mi" aria-hidden="true">home</i>
                    ...
                    @foreach($subcategories as $i => $sub)
                        <button data-sub="{{ $sub }}" aria-pressed="false">
                            <i class="mi" aria-hidden="true">
                                {{ ['yard','chair','light','kitchen','cleaning_services','deck'][$i] ?? 'home' }}
                            </i>
                            {{ $sub }}
                        </button>
                    @endforeach
            ...
            @include('buyer.Category.Home&Garden.components.home-garden-filter')
        </aside>
        ...
        <main id="bc-main" tabindex="-1">
            ...
            <p class="subtitle">
                Make every corner feel more comfortable, organized, and alive.
            </p>
    ...
    @include('buyer.Category.Home&Garden.components.home-garden-product-card')
