...
<section class="page-hero">
    ...
    <div class="hero-actions"><button class="button button-secondary" type="button" data-settings-reset><i data-lucide="rotate-ccw"></i> Reset preview</button></div>
</section>
...
<section class="settings-grid">
    <article class="panel settings-editor-panel">
        ...
        <div class="panel-footer-actions"><button class="button button-secondary" type="button" data-announcement-save-draft>Save draft</button><button class="button button-primary" type="button" data-announcement-publish><i data-lucide="megaphone"></i> Publish announcement</button></div>
    </article>
    ...
    <aside class="panel preview-panel">
        ...
        <div class="recent-announcements" data-recent-announcements><span class="section-label">Recent announcements</span>@foreach($announcements as $announcement)<div data-seed-announcement><strong>{{ $announcement['title'] }}</strong><small>{{ $announcement['audience'] }} • {{ $announcement['date'] }}</small><span class="status-badge {{ $announcement['status'] === 'Published' ? 'badge-success' : 'badge-info' }}">{{ $announcement['status'] }}</span></div>@endforeach</div>
    </aside>
</section>

<section class="panel policy-panel">
    <div class="panel-heading"><div><span class="eyebrow">Platform policies</span><h2>Terms & marketplace policy editor</h2><p>Last mock update: {{ $policy['updated'] }}</p></div><button class="button button-secondary button-small" type="button" data-policy-history-open><i data-lucide="history"></i> Version history</button></div>
    <div class="policy-editor-grid">
        <div><label class="form-field"><span>Policy title</span><input type="text" value="{{ $policy['title'] }}" data-policy-title></label><label class="form-field"><span>Policy content</span><textarea rows="12" data-policy-editor>{{ $policy['body'] }}</textarea></label><div class="panel-footer-actions"><button class="button button-secondary" type="button" data-policy-save-draft>Save draft</button><button class="button button-primary" type="button" data-policy-publish>Publish update</button></div></div>
        <div class="policy-preview"><span class="section-label">Rich-text preview</span><h3 data-policy-preview-title>{{ $policy['title'] }}</h3><p data-policy-preview>{{ $policy['body'] }}</p><div class="policy-note"><i data-lucide="info"></i><span>This is a static preview. No policy is actually being changed.</span></div></div>
    </div>
</section>
<div class="modal-shell" data-modal="policy-history" hidden>
    <button class="modal-backdrop" type="button" data-close-modal></button>
    <section class="modal-card" role="dialog" aria-modal="true" aria-label="Policy version history">
        <div class="modal-heading">
            <div>
                <span class="eyebrow">Platform policies</span>
                <h2>Version history</h2>
                <p>Saved and published policy versions in this front-end preview.</p>
            </div>
            <button class="icon-button" type="button" data-close-modal><i data-lucide="x"></i></button>
        </div>

        <div class="recent-announcements" data-policy-history-list>
            <span class="section-label">Policy versions</span>
        </div>
